...
def main():
	...
	for i in range (0, 200):
		#url = "http://www.zappos.com/womens-shoes~ov?zfcTest=sis%3A1#!/women-shoes/CK_XAXoC1ASCAQKhGMABAeICAwEYDw.zso?p=" + str(i) + "&s=recentSalesStyle/desc/"
                url = "http://www.zappos.com/womens-shoes~ov#!/women-shoes/CK_XAXoC1ASCAQKdGMABAeICAwEYDw.zso?p=" + str(i) + "&s=recentSalesStyle/desc/"
...
def parse_product_page(text):
	...
	if brand is None:
		brand = BS.find('a', class_="to-brand")
		if brand is None:
			return
		else:
			brand = brand.string.strip()
	else:
		brand = brand.get('alt')
	#print brand
